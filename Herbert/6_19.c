...
void setup()
{
  ...
  while (!OrangutanPushbuttons::isPressed(BUTTON_B))
  {
    ...
    // Display the position measurement, which w ill go from 0
  ...
  OrangutanLCD::print("Go! JPMB Bot");   
...
void follow_segment(unsigned int speed)
{
  ...
  while(1)
  {
    ...
    // to a negative value.
    const int maximum = speed; // the maximum speed
